...
# This code is to model Cornelissen softening using CZ-PFM for the L-shaped specimen. The mesh is generated in the code. 
...
Gc = 0.09                             # critical fracture energy
...
# Mesh Data
mesh = Mesh('L_shape_coarse.xml')       # import mesh as .xml
...
remain = CompiledSubDomain("x[1] <= 0.0 + tol", tol = 1E-14)                # here it is the bottom edge of the specimen   
...
tol_v = 1e-24
# Bottom edge to fix
def boundary_D_b(x, on_boundary):
    return x[1] <= 0.0 + tol

# Bottom right corner to fix
def boundary_D_f(x, on_boundary):
    return x[0] >= 250.0 - tol and x[1] <= 0.0 + tol

# Location to apply displacement
def boundary_D_t(x, on_boundary):
    return x[1] <= 250.0+tol and x[0] >= 469.9-tol and x[0] <= 470.1 +tol 
        
bcb = DirichletBC(V_u.sub(1), Constant(0.0), boundary_D_b, method='pointwise') # define boundary condition

bcf = DirichletBC(V_u, Constant((0.0,0.0)), boundary_D_f, method='pointwise') # define boundary condition
...
num_steps = 2647                     # total number of load steps
num_of_large_load_step = 7           # number of large load steps
total_disp = 0.8                     # total applied displacement


u_R = Expression(('disp_A + disp_app*(n+1-B)'),disp_A = 0.0, disp_app = total_disp/(num_of_large_load_step + 33), n=0., B=0., degree=0)          # Define loading as an expression so that it can be updated for next step
bct = DirichletBC(V_u.sub(1), u_R, boundary_D_t, method='pointwise')                   # define boundary condition
bc_disp = [bcb, bcf, bct]                                                              # Apply boundary conditions
...
prm["newton_solver"]["relative_tolerance"] = 2E-1
...
solver_alpha_tao.parameters.update({"method": "tron","linear_solver" : "umfpack","preconditioner" : "petsc_amg",
                                    "line_search": "gpcg", "report": False, "maximum_iterations": 10000, "gradient_absolute_tol": 8.0e-04, "gradient_relative_tol": 8.0e-04})
                                    
# Boundary conditions for phase field
def boundary_D_d_c(x, on_boundary):
     return x[1] <= 250.6+tol and x[0] >= 468.0-tol and x[0] <= 472.0 +tol 
...
def alternate_minimization(u,alpha,tol=8.e-4,maxiter=10000,alpha_0=interpolate(Constant("0.0"), V_alpha)):
    # initialization
    iter = 1; err_alpha = 1
...
def postprocessing():  
    forces[n+1] = np.array([u(470.,250.)[1],assemble(sigma(u,alpha)[1,1]*thickness*ds(1))])
...
for n in range(num_steps):
    if n > num_of_large_load_step-1:                # To switch from large steps to small steps
        u_R.disp_A = total_disp/(num_of_large_load_step+33)*num_of_large_load_step
        u_R.disp_app = 2.5e-4
    ...
    print("\nEnd of timestep %d with load %g"%(n, u(470.,250.)[1])) # print completion of load step in terminal
    print("-----------------------------------------")
